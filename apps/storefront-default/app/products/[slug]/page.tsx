  ...
  return (
    <SitePageFrame title={product.title} description={product.categories[0]?.name ?? '브랜드'}>
      ...
      <section style={{ paddingTop: 12 }}>
        <div style={{ width: 1472, margin: '0 auto', display: 'grid', gridTemplateColumns: '720px 1fr', gap: 56 }}>
          <div>
            <div
              style={{
                aspectRatio: '1 / 1',
          ...
          <div>
            ...
            <h1 style={{ margin: 0, fontSize: 40, lineHeight: 1.2, letterSpacing: '-0.04em', fontWeight: 900 }}>{product.title}</h1>
            <div style={{ marginTop: 18, display: 'flex', alignItems: 'baseline', gap: 12 }}>
              <strong style={{ fontSize: 36 }}>{product.price.active.formatted}</strong>
              {product.price.sale ? (
            ...
            <div style={{ marginTop: 22, padding: '18px 20px', background: '#f9fafb', border: '1px solid #e5e7eb', lineHeight: 1.8 }}>
              <div><strong>재고</strong> : {product.stock.quantity}</div>
              <div><strong>상태</strong> : {product.stock.quantity > 0 ? '구매 가능' : '품절/대기'}</div>
            </div>

            <form action={addToCartAction} style={{ display: 'flex', gap: 12, alignItems: 'center', marginTop: 24 }}>
